...
const randomDice = () => {
  ...
  setTimeout(() => {
    moveFigure(currentPlayer, rollValue);
    togglePlayer();
    enableRollButton(); // Re-enable the roll button here
...
rollBtn.addEventListener("click", () => {
  rollBtn.disabled = true; // Disable the button to prevent re-clicking
  randomDice();
  socket.emit('rollDice', { player: currentPlayer, rollValue });
});

const enableRollButton = () => {
  rollBtn.disabled = false; // Re-enable the button after move
};


// Listen for the unified updateMove event from the server
socket.on('updateMove', (data) => {
  // Update the dice roll display
  rollDice(data.rollValue);

  // Move the figure to the new position
  moveFigure(data.player, data.newPosition);
});
...
const moveFigure = (player, roll) => {
  ...
  socket.emit('movePiece', { player, newPosition: playerPositions[player].position.id });
};
